...
ckpt_path='./ckpt/CPPNx1/checkpoint.ckpt'
...
def get_fitness(g, inp, CLASS, k):
    global gen
    save_path='./CPPNGenerated/CPPNx2/class{}'.format(CLASS)
    if pop.generation==0:
        gen=pop.generation
...
while 1:
    if len(os.walk('./CPPNGenerated/CPPNx2/class{}'.format(clazz)).next()[2])<1000:
