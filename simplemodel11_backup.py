...
for CLASS in range(10):
    numberoffiles = numberoffiles + len(os.walk('./CPPNGenerated/CPPNx1/class{}'.format(CLASS)).next()[2])
...
for CLASS in range(10):
     files_in_dir = os.listdir('./CPPNGenerated/CPPNx1/class{}'.format(CLASS))
     for files in files_in_dir:          
         data = np.load(os.path.join('./CPPNGenerated/CPPNx1/class{}'.format(CLASS), files))
         array = data['features']
         array = array[0,0,:,:]
...
fn = saver.save(sess, "%s/%s.ckpt" % ('./ckpt/CPPNx1', snapshot_name))
